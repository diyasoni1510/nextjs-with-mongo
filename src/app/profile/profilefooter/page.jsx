...
const ProfileFooter = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async ({ post, caption }) => {
      ...
      const userId = await axios.post("/api/users/getuserfromusername",{username:localStorage.getItem("username")})
      const uploadPost = await axios.post("/api/users/uploadpost",{post:base64String,caption,user:userId.data.data._id})
      setLoading(false)
      setIsUpload(false)
      ...
      {isUpload && (
        <dialog className="modal mt-16 z-40 w-full flex justify-center  bg-black bg-opacity-20 h-[620px] absolute top-0 md:top-[-40px]">
          <div className="modal-box w-full flex  justify-center items-center">
            <form className="flex space-y-3 flex-col justify-center items-center bg-white px-2 py-5 w-5/6" onSubmit={handleSubmit}>
              <input type="file" name="post" id="post" onChange={(e)=>{formik.setFieldValue('post',e.currentTarget.files[0])}} accept="image/*" className="hidden" ref={inputFileRef}/>
              <button type="button" className="w-[70px] h-[70px] rounded-full bg-gray-200 flex justify-center items-center" onClick={()=>{inputFileRef.current.click()}}><FiUpload className="text-3xl text-gray-400"/></button>
              {console.log(values.post)}
              { values.post ? values.post.name : "" }
              <label htmlFor="caption">Caption</label>
              <textarea name="caption" id="caption" rows={5} value={values.caption} onChange={handleChange} className="border border-gray-500 rounded-md w-full" />
              <button
                type="submit"
